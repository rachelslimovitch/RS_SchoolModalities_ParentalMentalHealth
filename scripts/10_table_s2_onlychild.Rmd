...
Code creates Supp. Table 2 (LPMs, December-April) for parents with only 1 child.
...
library(lubridate)
library(multcomp)
here::i_am("RS_Thesis_Code/10_table_s2_onlychild.Rmd")
```
...
v3_simple<- c("modality_in_person_1_0", "modality_part_time_1_0", "week_no", "REGION", "week_no * REGION",
              "urban_rural", "gender", "age", "quartile_white", "quartile_black_aa", "quartile_hisp_latino",
              "financial_worry", "education_level", "employment", "adult_cat", "worried_covid", 
...
Survey design setup:
```{r}
svy_associational<- svydesign(ids = ~1, weights = ~weight, data=data_single)

#south
data_single_south<- data_single %>%
  dplyr::filter(REGION=="South")
svy_associational_south<- svydesign(ids = ~1, weights = ~weight, data = data_single_south)

#northeast
data_single_northeast<- data %>%
  filter(REGION=="Northeast")
svy_associational_northeast<- svydesign(ids = ~1, weights = ~weight, data=data_single_northeast)

#west
data_single_west<- data %>%
  filter(REGION=="West")
svy_associational_west<- svydesign(ids = ~1, weights = ~weight, data = data_single_west)

#midwest
data_single_midwest<- data %>%
  filter(REGION=="Midwest")
svy_associational_midwest<- svydesign(ids = ~1, weights = ~weight, data = data_single_midwest)
```
